...
<c:if test="${next!=null}">
    ...
    <li onclick="location.href = '<c:url value="/BoardViewAction.do?&table=${table}&num=${nextNum}"/>'">
        <span>이전</span><span><%=next.getSubject()%></span><span class="date"><%=next.getRegist_day()%></span>
    </li>
</c:if>
...
<c:if test="${prev!=null}">
    ...
    <li onclick="location.href = '<c:url value="/BoardViewAction.do?&table=${table}&num=${prevNum}"/>'">
    <span>다음</span><span><%=prev.getSubject()%></span><span class="date"><%=prev.getRegist_day()%></span>
    </li>
</c:if>
    </ul>
    <button onclick="location.href = '<c:url value="/BoardListAction.do?table=${table}&pageNum=1"/>'">목록</button>
    </div>
    </section>
    </main>
